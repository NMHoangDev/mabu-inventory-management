...
export function createZaloAccount(payload: {
  ...
  return requestJson<{ account_id: string; message?: string }>("/api/zalo/accounts", {
    method: "POST",
...
export function syncZaloConversationMessages(
  ...
  return requestJson<{ ok: boolean; message: string }>(
    `/api/zalo/conversations/${encodeURIComponent(conversationId)}/sync?${params.toString()}`,
    {
      method: "POST",
      headers: {
        "X-User-ID": accountId,
...
export function deleteZaloAccountFull(
  ...
  }>("/api/zalo/auth/delete-account-full", {
    method: "POST",
    headers: buildHeaders({
...
export function cleanupZaloOrphanAccounts(): Promise<{
  ...
  return requestJson<{
    ...
    kept: any[];
    kept_count: number;
  }>("/api/zalo/auth/cleanup-orphan-accounts", {
    method: "POST",
    headers: buildHeaders({
      "X-User-ID": "admin",
    }),
...
export function buildZaloRealtimeStreamUrl(
  ...
  if (options.userId) params.set("user_id", options.userId);
  if (options.email)  params.set("email", options.email);
  // Single-user Mabu: always admin role, sees all accounts
  params.set("role", "admin");
  const qs = params.toString();
  // Route through Next.js catch-all proxy → Python Backend SSE (no absolute URL needed)
  return `/api/zalo/events/stream${qs ? `?${qs}` : ""}`;
}
...
export async function getZaloConversationShareStatus(
  ...
  return requestJson<ZaloShareStatus>(
    `/api/zalo/events/share-status?${params.toString()}`,
    {
      method: "GET",
...
export async function setZaloConversationShare(
  ...
  return requestJson<{ ok: boolean }>("/api/zalo/events/share", {
    method: "POST",
    headers: buildHeaders({
      "X-User-ID": userId,
      "Content-Type": "application/json",
...
export function findZaloUser(
  ...
  return requestJson<ZaloFoundUser>(
    `/api/zalo/conversations/users/find?${params.toString()}`,
    {
      method: "GET",
      headers: {
        "X-User-ID": accountId,
...
export function createZaloUserThread(
  ...
  return requestJson<ZaloCreateUserThreadResponse>(
    "/api/zalo/conversations/users/threads",
    {
      method: "POST",
      headers: buildHeaders({
        "X-User-ID": accountId,
