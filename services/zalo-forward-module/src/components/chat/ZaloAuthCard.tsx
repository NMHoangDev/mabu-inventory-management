...
/**
 * Ported nguyên văn từ components/zalo/chat/ZaloAuthCard.tsx (app chính) —
 * không đổi gì, chỉ đổi import type (@/lib/zaloApiClient thay vì
 * @/lib/zalo-api).
 */
    ...
    <div className="mb-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm">
        ...
        <div className="mt-2 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-2.5 text-[11px] text-slate-600">
          ...
          <ol className="ml-4 list-decimal space-y-0.5">
            <li>
              ...
              về máy, giải nén. Mở <code className="rounded bg-slate-200 px-1 font-mono text-[10px]">chrome://extensions</code>, bật Developer mode, rồi <strong>kéo thả thư mục vừa giải nén thẳng vào trang đó</strong> (hoặc bấm <strong>Load unpacked</strong> trỏ vào thư mục — 2 cách cho cùng kết quả). Lưu ý: Chrome không cài trực tiếp từ file .zip còn nén — phải giải nén trước.
            </li>
            <li>
              Mở tab{" "}
              <a href="https://chat.zalo.me/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-0.5 font-semibold text-blue-600 hover:underline">
                chat.zalo.me <ExternalLink className="h-3 w-3" />
              </a>{" "}
              và đăng nhập QR trên điện thoại.
            </li>
            <li>
              <strong className="text-amber-700">Quan trọng:</strong> trước khi bấm &quot;Đăng nhập Zalo&quot; ở đây, hãy <strong>đóng tab chat.zalo.me</strong> — nếu không Zalo sẽ đóng WS bridge ngay (lỗi Overlimit).
            </li>
            <li>
              Quay lại đây bấm <strong>Đăng nhập Zalo</strong> — extension tự lấy cookie và gửi về zalo-bridge.
            </li>
          </ol>
        </div>
      )}
      ...
      {isLoggedIn && (
        <div className="mt-1.5 rounded-md border border-amber-200 bg-amber-50 px-2.5 py-1.5 text-[11px] text-amber-800">
          <strong>Lưu ý:</strong> để WS không bị đóng bởi Zalo (lỗi <code className="font-mono">Overlimit connection</code>), hãy đóng tab chat.zalo.me khi dùng UI này. Nếu lỡ mở, bấm <strong>Reconnect WS</strong>.
        </div>
      )}

      {error && (
        <div className="mt-1.5 rounded-md border border-red-200 bg-red-50 px-2.5 py-1.5 text-[11px] text-red-700">⚠️ {error}</div>
      )}

      {isLoggedIn && (
        <div className="mt-1.5 flex items-center gap-1.5 text-[11px] text-emerald-600">
          <CheckCircle2 className="h-3 w-3" />
          <span>Sẵn sàng gửi và nhận tin nhắn.</span>
        </div>
      )}
    </div>
  );
