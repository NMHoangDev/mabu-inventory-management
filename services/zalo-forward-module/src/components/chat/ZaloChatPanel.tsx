...
/**
 * Ported nguyên văn từ components/zalo/chat/ZaloChatPanel.tsx (app chính) —
 * không đổi logic, chỉ đổi import type (@/lib/zaloApiClient).
 */
...
function groupImageMessagesForDisplay(messages: ZaloMessage[]): ZaloMessage[] {
  ...
  for (const m of messages) {
    ...
    const withinWindow = !!last && Math.abs(new Date(m.timestamp || 0).getTime() - new Date(last.timestamp || 0).getTime()) <= IMAGE_GROUP_WINDOW_MS;

    if (last && isImageOnlyMessage(m) && isImageOnlyMessage(last) && last.sender_id === m.sender_id && last.is_sent === m.is_sent && withinWindow) {
...
function formatTime(dateStr?: string | null): string {
  ...
  return t.toLocaleString("vi-VN", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit" });
}
  ...
  if (msg.is_deleted) {
    return (
      <div className={`flex ${mine ? "justify-end" : "justify-start"}`}>
        <div className="rounded-2xl bg-slate-100 px-4 py-2 text-xs italic text-slate-400">Tin nhắn đã bị thu hồi</div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex gap-2 ${mine ? "justify-end" : "justify-start"}`}>
      ...
      <div className={`max-w-[80%] ${mine ? "items-end" : "items-start"} flex flex-col gap-0.5`}>
        {!mine && msg.sender_name && <div className="px-2 text-[11px] font-semibold text-slate-500">{msg.sender_name}</div>}
        <div className={`rounded-2xl px-4 py-2 text-sm leading-relaxed ${mine ? "rounded-br-sm bg-blue-600 text-white" : "rounded-bl-sm bg-white text-slate-800 shadow-sm"}`}>
          {msg.image_urls && msg.image_urls.length > 0 && (
            <div className={`grid gap-1 ${msg.image_urls.length > 1 ? "grid-cols-2" : ""}`}>
              {msg.image_urls.map((url, i) => (
                <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={url} alt={`image-${i}`} className="max-h-56 max-w-full rounded-lg object-cover transition hover:opacity-90" />
                </a>
              ))}
            </div>
          )}
          {msg.content && (
            <div className={`whitespace-pre-wrap break-words ${msg.image_urls && msg.image_urls.length > 0 ? "mt-2" : ""}`} onClick={() => setExpanded(!expanded)}>
              {msg.content}
            </div>
          )}
        </div>
        <div className={`px-2 text-[11px] text-slate-400 ${mine ? "text-right" : ""}`}>{formatTime(msg.timestamp || msg.time_text)}</div>
      </div>
    </div>
  );
}

export function ZaloChatPanel({ conv, messages, loading, sending, replyText, setReplyText, pendingFiles, setPendingFiles, onSend, onSync }: Props) {
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50 p-4">
        {loading && messages.length === 0 ? (
          ...
          <div className="flex h-full items-center justify-center text-sm text-slate-400">Chưa có tin nhắn nào. Hãy bắt đầu cuộc trò chuyện!</div>
        ) : (
          <div className="space-y-3">
            {displayMessages.map((m, idx) => {
              const tsRaw = m.timestamp ? new Date(m.timestamp).getTime() : 0;
              const mId = m.message_id ? String(m.message_id) : `${Number.isFinite(tsRaw) ? tsRaw : idx}|${m.sender_id ?? ""}|${(m.content ?? "").slice(0, 50)}`;
      ...
      {pendingFiles.length > 0 && (
        <div className="flex shrink-0 flex-wrap gap-2 border-t border-slate-200 bg-slate-50 p-2.5">
          {pendingFiles.map((f, i) => (
            <div key={i} className="flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2.5 py-1 text-xs">
              {f.type.startsWith("image/") ? <ImageIcon className="h-3.5 w-3.5 text-blue-500" /> : <Paperclip className="h-3.5 w-3.5 text-slate-500" />}
              <span className="max-w-[140px] truncate">{f.name}</span>
              <button onClick={() => setPendingFiles(pendingFiles.filter((_, idx) => idx !== i))} className="ml-1 text-slate-400 hover:text-red-500">
      ...
      <div
        className={`flex shrink-0 items-end gap-2 border-t border-slate-200 p-3 ${dragOver ? "bg-blue-50" : "bg-white"}`}
        onDragOver={(e) => {
          e.preventDefault();
          setDragOver(true);
        }}
      ...
      >
        ...
        <button onClick={() => fileInputRef.current?.click()} className="grid h-10 w-10 shrink-0 place-items-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-blue-600" title="Đính kèm file">
