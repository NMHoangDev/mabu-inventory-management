 ...
 *
 * Bản copy trong zalo-forward-module (khác account-module): nhận thêm
 * `initialThreadId` — khi vào từ nút "Chat" ở 1 rule cụ thể (trỏ đúng
 * master_thread_id của rule đó), tự mở luôn conversation đó ngay khi đăng
 * nhập xong, không bắt user phải tự tìm trong danh sách hội thoại.
 */
...
export function ZaloPageContent({ accountId, initialThreadId }: { accountId: string; initialThreadId?: string }) {
  ...
  useEffect(() => {
    if (!initialThreadId || openedInitialRef.current || !z.isLoggedIn) return;
    openedInitialRef.current = true;
    void z.openConversation(initialThreadId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initialThreadId, z.isLoggedIn]);
