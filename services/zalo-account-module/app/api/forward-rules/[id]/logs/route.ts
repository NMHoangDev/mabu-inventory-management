/**
 * GET /api/forward-rules/[id]/logs?limit=30 — port từ
 * app/api/zalo/forward-rules/[id]/logs/route.ts của app chính.
 */
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await sb(`/zalo_forward_logs?rule_id=eq.${ruleId}&select=*&order=created_at.desc&limit=${limit}`);
