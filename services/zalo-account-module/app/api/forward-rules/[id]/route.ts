/**
 * PATCH/DELETE /api/forward-rules/[id] — port từ
 * app/api/zalo/forward-rules/[id]/route.ts của app chính.
 */
...
async function loadExistingRuleGraph(accountId: string, excludeRuleId: number) {
  ...
  const rules: Array<{ id: number; master_thread_id: string }> = rulesRes.ok ? await rulesRes.json() : [];
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (targets || body?.master_thread_id != null) {
      const { existingMasters, existingTargetsFlat } = await loadExistingRuleGraph(existingRule.account_id, ruleId);
      const checkTargets = targets ?? [];
      if (!targets) {
        const currentTargetsRes = await sb(`/zalo_forward_targets?rule_id=eq.${ruleId}&select=target_thread_id`);
        const rows: Array<{ target_thread_id: string }> = currentTargetsRes.ok ? await currentTargetsRes.json() : [];
    ...
    if (body?.master_thread_name !== undefined) {
      updateRow.master_thread_name = body.master_thread_name ? String(body.master_thread_name).trim() : null;
    }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [existingRule] = existingRes.ok ? await existingRes.json() : [];
  if (!existingRule) return NextResponse.json({ ok: true });
