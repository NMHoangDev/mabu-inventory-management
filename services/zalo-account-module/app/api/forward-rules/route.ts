/**
 * GET  /api/forward-rules?account_id=shop-owner
 * POST /api/forward-rules
 *
 * Port nguyên logic từ app/api/zalo/forward-rules/route.ts của app chính —
 * cùng đọc/ghi bảng Supabase zalo_forward_rules/targets, nên rule tạo ở đây
 * hay ở app chính đều nhất quán (không có 2 nguồn sự thật).
 */
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const targetsRes = await sb(`/zalo_forward_targets?rule_id=in.(${ruleIds})&select=*&order=created_at.asc`);
    ...
    const result = rules.map((r: { id: number }) => ({ ...r, targets: byRule.get(r.id) || [] }));
...
export async function POST(req: NextRequest) {
  ...
  if (staff.role !== "admin" && !canBroadcastTo(staff, accountId)) {
    return NextResponse.json({ error: "Bạn không có quyền cấu hình chuyển tiếp cho tài khoản này" }, { status: 403 });
  }
