/**
 * Ported nguyên văn từ app/api/zalo/threads/[threadId]/read/route.ts (app
 * chính) — mark read khi mở conversation. Không đổi logic.
 */
...
async function pickKey(): Promise<{ key: string; isService: boolean } | null> {
  ...
  for (const c of cands) {
    try {
      const r = await fetch(`${SUPABASE_URL}/rest/v1/zalo_messages?select=source_message_id&limit=1`, {
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ threadId: string }> }) {
  try {
    ...
    if (!picked) return NextResponse.json({ ok: false, error: "no valid key" }, { status: 200 });
    const supabase = createClient(SUPABASE_URL!, picked.key, { auth: { persistSession: false, autoRefreshToken: false } });

    const { threadId } = await ctx.params;
    const body = await req.json().catch(() => ({}));
    const accountId: string = body?.account_id || "shop-owner";

    if (!threadId) return NextResponse.json({ ok: false, error: "missing threadId" }, { status: 200 });

    const { error } = await supabase.from("zalo_conversations_ui").update({ unread_count: 0, updated_at: new Date().toISOString() }).eq("account_id", accountId).eq("thread_id", threadId);
