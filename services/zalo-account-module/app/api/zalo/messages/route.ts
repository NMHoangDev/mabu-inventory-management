/**
 * Ported nguyên văn từ app/api/zalo/messages/route.ts (app chính) — đọc/ghi
 * bảng zalo_messages, dùng cho phần chat mới của module này. Không đổi logic.
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!picked) return NextResponse.json({ messages: [], total: 0, note: "no valid key" });
    ...
    if (!threadId) return NextResponse.json({ messages: [], total: 0, note: "missing thread_id" });

    const { data, error } = await supabase.from("zalo_messages").select("*").eq("user_id", accountId).eq("thread_id", threadId).order("ts", { ascending: true }).limit(limit);

    if (error) return NextResponse.json({ messages: [], total: 0, note: error.message });
    ...
    for (const r of rows) {
      ...
      if (isStableId) {
        ...
        if (existing) {
          const winner = String(r.content ?? "").length > String(existing.content ?? "").length ? r : existing;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!threadId || messages.length === 0) return NextResponse.json({ ok: true, upserted: 0 });

    const rows = messages.filter((m) => m?.content !== undefined || m?.message_id).map((m) => toRow(m, accountId, threadId, threadType));

    const { data, error } = await supabase
      .from("zalo_messages")
      .upsert(rows, { onConflict: "user_id,source_message_id", ignoreDuplicates: insertOnly })
    ...
    try {
      ...
      const nextUnread = lastRow.is_sent ? 0 : (cur?.unread_count || 0) + 1;
      const effectiveThreadType = cur?.thread_type === "group" ? "group" : threadType;
      const fallbackName = effectiveThreadType === "group" ? `Group ${threadId}` : `Zalo ${threadId}`;

      await supabase.from("zalo_conversations_ui").upsert(
        {
          account_id: accountId,
          thread_id: threadId,
          ...
          conversation_name:
            (typeof (lastRaw as Record<string, unknown>)?.display_name === "string" && (lastRaw as Record<string, unknown>).display_name) || cur?.conversation_name || fallbackName,
