...
type PaymentType = "" | "order_payment" | "supplier_payment" | "other";
...
const PAYMENT_CATEGORY_LABEL: Record<PaymentType, string> = {
  "": "Tự động",
...
export default function PaymentVouchersPage() {
  ...
  const fetchList = useCallback(async () => {
    ...
    setError("");
    try {
      const params = new URLSearchParams({ voucher_type: "payment", page: String(page), pageSize: String(pageSize) });
    ...
    <div className="flex flex-col h-[calc(100vh-4.5rem)] -m-4 lg:-m-6">
      ...
      <div className="px-4 py-3 border-b bg-gray-50 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-4 text-sm text-slate-600">
          <button
            onClick={() =>
              downloadCsv(`phieu-chi-${Date.now()}.csv`, rows, [
                { label: "Ngày tạo", value: (r) => formatDateTime(r.created_at) },
                { label: "Mã phiếu", value: (r) => r.code },
                { label: "Loại phiếu", value: (r) => PAYMENT_CATEGORY_LABEL[r.payment_type] ?? r.payment_category ?? "Tự động" },
                { label: "Trạng thái", value: (r) => STATUS_META[r.status]?.label ?? r.status },
                { label: "Số tiền chi", value: (r) => r.amount },
                { label: "Nhóm người nhận", value: (r) => r.group_name },
                { label: "Chứng từ gốc", value: (r) => r.reference_code },
                { label: "Tên người nhận", value: (r) => r.person_name },
              ])
            }
      ...
      <div className="p-4 flex items-center gap-2 bg-white border-b flex-shrink-0">
        ...
        <button className="px-4 py-2 border border-gray-300 rounded-md text-sm text-slate-700 bg-white hover:bg-gray-50 flex items-center">
          Nhóm người nhận <svg className="ml-2 w-3 h-3 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7" strokeWidth="2" strokeLinecap="round" /></svg>
        </button>
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                <th className="px-4 py-3 border-b w-10"><input type="checkbox" className="rounded border-gray-300 text-blue-600" /></th>
                <th className="px-4 py-3 border-b">Ngày tạo <span className="inline-block translate-y-px">↕</span></th>
                <th className="px-4 py-3 border-b">Mã phiếu</th>
                <th className="px-4 py-3 border-b">Loại phiếu</th>
                <th className="px-4 py-3 border-b text-center">Trạng thái</th>
                <th className="px-4 py-3 border-b text-right">Số tiền chi</th>
                <th className="px-4 py-3 border-b">Nhóm người nhận</th>
                <th className="px-4 py-3 border-b">Chứng từ gốc</th>
                <th className="px-4 py-3 border-b">Tên người nhận</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-100 text-slate-700">
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="p-12 text-center text-slate-500">
                    {total === 0 && !search
                      ? "Chưa có phiếu chi nào. Bấm \"Tạo phiếu chi\" để bắt đầu."
                ...
                rows.map((row) => {
                  ...
                  const categoryLabel = PAYMENT_CATEGORY_LABEL[row.payment_type] ?? row.payment_category ?? "Tự động";
                  return (
                    <tr key={row.id} className="hover:bg-blue-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/finance/payment-vouchers/${row.id}`} className="text-blue-600 font-medium hover:underline">
                          {row.code}
                        </Link>
                      </td>
      ...
      <div className="px-6 py-4 bg-blue-50 border-t flex justify-center">
