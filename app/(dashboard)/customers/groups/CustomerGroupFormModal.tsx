...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  initialData?: (CustomerGroupFormData & { id: string });
}

const GROUP_TYPES = ["Cố định", "Động"];

function emptyForm(): CustomerGroupFormData {
  return { name: "", code: "", type: "Cố định", description: "" };
}

export function CustomerGroupFormModal({ open, onClose, onSuccess, initialData }: Props) {
  const [form, setForm] = useState<CustomerGroupFormData>(emptyForm());
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name.trim()) {
      setError("Tên nhóm khách hàng là bắt buộc.");
      return;
    }
    setSaving(true);
    setError("");
    try {
      const res = isEdit
        ? await fetch(`/api/customer-groups/${initialData!.id}`, {
            method: "PATCH",
          ...
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Nhập mô tả cho nhóm khách hàng"
                rows={3}
