...
export default function CustomerDebtPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/reports/debt?type=customer", { cache: "no-store" });
    ...
    >
      ...
      <div className="rounded-lg border border-gray-100 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-5 py-4">
          <h3 className="text-sm font-semibold text-gray-800">Danh sách khách hàng còn nợ</h3>
        </div>
        <ReportTable
          empty="Không có khách hàng nào còn nợ tiền hàng."
          columns={[
