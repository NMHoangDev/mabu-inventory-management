...
export default function PaymentByBranchPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const range = period === "custom" && dateFrom && dateTo
        ...
        : getDateRange(period);
      const result = await fetchPaymentByBranch(range);
      setData({ branches: result.branches, summary: result.summary });
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  return (
    <ReportShell
      title="Báo cáo thanh toán nhập hàng theo chi nhánh"
        ...
        <>
          ...
          {view === "chart" && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              <div className="bg-white rounded-lg border border-gray-100 p-5 shadow-sm">
                <h3 className="font-semibold text-gray-800 text-sm mb-4">Chi theo chi nhánh</h3>
                <DonutChart
                  data={d.branches.map((b, i) => ({
                    label: b.branch,
          ...
          <div className="bg-white rounded-lg border border-gray-100 shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100">
              <h3 className="font-semibold text-gray-800 text-sm">Chi tiết theo chi nhánh</h3>
            </div>
            <ReportTable
              columns={[
