  ...
  const load = useCallback(async () => {
    ...
    try {
      const range = period === "custom" && dateFrom && dateTo
        ...
        : getDateRange(period);
      const result = await fetchAboveThreshold(range);
      setData({ items: result.items, total_excess: result.summary.total_excess, capital_locked: result.summary.capital_locked });
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
        ...
        <>
          ...
          {view === "chart" && (
            <div className="bg-white rounded-lg border border-gray-100 p-5 shadow-sm mb-6">
              <h3 className="font-semibold text-gray-800 text-sm mb-4">So sánh tồn kho / tồn tối đa</h3>
              <SvgBarChart
                labels={d.items.slice(0, 10).map((i) => i.sku)}
                datasets={[
                  { label: "Tồn hiện tại", data: d.items.slice(0, 10).map((i) => i.current_qty), color: "#f59e0b" },
                  { label: "Tồn tối đa", data: d.items.slice(0, 10).map((i) => i.max_stock), color: "#22c55e" },
                ]}
                height={200}
              />
            </div>
          ...
          <div className="bg-white rounded-lg border border-gray-100 shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100">
              <h3 className="font-semibold text-gray-800 text-sm">Danh sách sản phẩm vượt định mức</h3>
            </div>
            <ReportTable
              columns={[
