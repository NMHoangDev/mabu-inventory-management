...
async function main() {
  ...
  if (args.command === "user-history") {
    const userId = args["user-id"] || input.user_id || input.userId;
    const count = Math.max(50, Math.min(Number(args.count || input.count || 1000), 3000));
    ...
    // Ép cứng thread = userId để tránh lỗi Zalo trả nhầm threadId
    messages.forEach(m => {
      m.group_id = String(userId);
      m.thread_id = String(userId);
    });
    ...
    try {
      ...
      if (includeFriends && summary.friends.length > 0) {
        ...
        for (let i = 0; i < friendIds.length; i += CONCURRENCY) {
          ...
          const results = await Promise.allSettled(
            chunk.map(async (fid) => {
              // Lấy lịch sử cá nhân (DM) sử dụng getUserChatHistory
