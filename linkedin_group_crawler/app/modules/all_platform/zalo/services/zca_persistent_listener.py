...
_ZALO_API_SEMAPHORE: Optional[asyncio.Semaphore] = None


def _get_zalo_api_semaphore() -> asyncio.Semaphore:
    global _ZALO_API_SEMAPHORE
    if _ZALO_API_SEMAPHORE is None:
        _ZALO_API_SEMAPHORE = asyncio.Semaphore(2)
    return _ZALO_API_SEMAPHORE
...
class ZcaPersistentListenerManager:
            ...
            try:
                ...
                for idx, group_id in enumerate(group_ids):
                    ...
                    try:
                        ...
                        async with _get_zalo_api_semaphore():
                            if is_group:
    ...
    async def _handle_event(self, state: ListenerState, raw_line: str) -> None:
        ...
        if event_name in {"disconnected", "closed", "stopping"}:
            logger.warning(f"ZCA listener disconnected/closed user={state.user_id} event={event_name}")
            state.connected = False
            return
