...
class SyncDomConversation(BaseModel):
    ...
    is_friend: Optional[bool] = None  # Nhận từ extension scrape — phân biệt DM vs nhóm
...
async def sync_dom_messages(
    ...
    """
    user_id = _normalize_user_id(body.account_id or x_user_id)
    # Bug #9 fix: phải có account_id hợp lệ — nếu để trống sẽ lưu tin vào account 'default' không tồn tại
    if user_id == "default":
        from fastapi import HTTPException
        raise HTTPException(status_code=400, detail="account_id là bắt buộc cho sync-dom")
    results: List[SyncRecentGroupResult] = []
    total_saved = 0
    groups_with_messages = 0
    errors = 0
        ...
        try:
            ...
            if messages:
                ...
                await upsert_group(
                    ...
                    is_friend=getattr(conversation, "is_friend", None),  # safe attr access
                )
            total_saved += saved
            if saved > 0:
                groups_with_messages += 1
