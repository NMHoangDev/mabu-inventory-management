...
const MSG_MANUAL_OPENED =
  "Đã mở màn hình Zalo remote. Hãy đăng nhập hoặc xử lý xác minh bổ dung rồi quay lại màn monitor.";
const MSG_QR_READY =
...
const appUser = { id: "default", email: "admin@localhost", role: "admin" };
...
export function useZaloCrawlerFlow(): ZaloCrawlerFlowValue {
  ...
  const switchWorker = useCallback((_workerId: string) => {
    // Worker routing is automatic by user_id
  }, []);
  ...
  useEffect(() => {
    ...
    const streamUrl = `/api/zalo/auth/events?${streamParams.toString()}`;
